...
class Action:
    def __init__(self, username=None, password=None, repo=None, branch=None, directory=None, https=True):
...
if __name__ == '__main__':
    ...
    parser.add_argument('--username', '-u', type=str, default=None, help='gitee username')
    parser.add_argument('--password', '-p', type=str, default=None, help='gitee password')
    parser.add_argument('--repo', '-r', type=str, default="double12gzh/wiki", help='gitee repository')
    parser.add_argument('--branch', '-b', type=str, default=None, help='gitee branch. Default: master')

    args = parser.parse_args()

    repo = os.environ.get('GITEE_REPO', None) if not args.repo else args.repo
    username = os.environ.get('GITEE_USERNAME', None) if not args.username else args.username
    password = os.environ.get('GITEE_PASSWORD', None)if not args.password else args.password
    branch = os.environ.get('GITEE_BRANCH', 'master') if not args.branch else args.branch

    https = os.environ.get('GITEE_HTTPS', True)
    directory = os.environ.get('GITEE_DIRECTORY') or ''

    if not username or not password:
        parser.print_help()
        print("\nWithout any input paramters, you can also set \r\n\r\n\tGITEE_USERNAME \r\n\tGITEE_PASSWORD \r\n\tGITEE_BRANCH\t(Default: master) \r\n\tGITEE_REPO\t(Default: double12gzh/wiki) \r\n\r\nin env.")
        exit(-1)

    Action(username=username, password=password, repo=repo, branch=branch, https=https, directory=directory)()
